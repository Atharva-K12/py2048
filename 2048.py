...
def rotation_anti(board,n):# rotates the complete board in anti clockwise sense
	...
	return board
def rotation(board,n):#rotates the complete board in clockwise sense
	for i in range(n // 2):
		for j in range(i, n-i-1):
			board[i][j],board[n-1-j][i],board[n-1-i][n-1-j],board[j][n-1-i]=\
			board[n-1-j][i],board[n-1-i][n-1-j],board[j][n-1-i],board[i][j]
	return board
def print_board(board,n):#printing board
    for i in range(n):
        for j in range(n):
            print(board[i][j],end="  ")
        print()
def clr():#for clearing screen
  if os.name=="nt" :
    os.system('cls')
  else:
    os.system('clear')
...
def adding(board,n):#adds the similar value,addition starts from the left end of board like in the actual game,sets a flag if nothing is changed
	...
	if temp==board:
		flag=1
	return (board,flag)
def new_2(board,n):#genarates random 2 on location with zero,non zero locations aren't affected
	while True:
...
def check(board,n,w):#fn to check win,loss,returns zero if zero(s) is/are present on Board,return end if won /lost,returns cons is zeros are exhausted but moves are still possible
	for i in range(n):
		if w in board[i]:
			clr()
			print_board(board,n)
	...
	print("You lost")
	return "end"
def game_2048(board,n,w,flag):#this fn does the main work
	ret=check(board,n,w)
	if ret!="end":
		if ret=="zero" and flag==0:#flag is false if there was changes in moving and ret is zero if board has zeros left
			board=new_2(board,n)
		else:
